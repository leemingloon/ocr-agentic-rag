...
- **Credit risk:** ipynb files in [notebooks/](notebooks/) [Completed]
  ...
  - PD predictions in [data/proof/credit_risk_pd/](data/proof/credit_risk_pd/) on Hugging Face pre-defined splits (train/valid/test) with AUC scores across models (Optuna-tuned or untuned):
    - [tuned-LR](data/proof/credit_risk_pd/lendingclub_lr_tuned/lendingclub_lr_tuned_avg.json) [Selected for OOT AUC (rank-ordering, scorecard, risk segmentation)]
    - [tuned-XGB/untuned-LGB stack](data/proof/credit_risk_pd/lendingclub/lendingclub_avg.json) [Selected for calibration (int rate pricing, capital provisioning)]
