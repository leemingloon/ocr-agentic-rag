...
**Update (2026-09-02): the "low scores" below are resolved — the committed proof was stale, not the pipeline.** The `data/proof/ocr/**` JSON checked into this repo up to that date held predictions from an old pipeline run (some FUNSD samples had almost no recognized text) and, separately, an aggregation bug that wrote literal `0.0`/missing `*_mean` into `*_avg.json` and `eval_summary.json` regardless of the per-sample predictions. Running `python eval_runner.py --all_ocr_splits --force_reeval` against the current code reproduces the pipeline honestly and gives **FUNSD word_recall_mean 0.971, SROIE entity_match_mean 0.960** (see `EVALUATION_RESULTS.md`) — both well above the "acceptable" 0.6–0.7 bar this document targets below. A second, real bug was found and fixed while verifying this: `PaddleOCRDetector._detect_native` (the confidence-gated, non-`force_paddleocr` box/router path used outside eval) called PaddleOCR with `rec=False`, which crashes on paddleocr 2.7.3 and silently degraded to near-empty text — see `ocr_pipeline/detection/paddleocr_detector.py`. `scripts/ocr_eval_improve_loop.py` is a regression-guard loop: it re-checks the weighted metric after any eval run and, only if a dataset drops below 0.70, escalates the worst-scoring samples to a direct Claude-vision reading and re-checks — so a future regression here gets caught and auto-remediated rather than silently re-shipped as another stale proof file.
