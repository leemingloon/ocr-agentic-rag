...
def parse_accounts_from_query(query: str) -> List[str]:
    ...
    if not query or not isinstance(query, str):
        return []
    q = query.strip()
    # "of X and Y" or "of X, Y" or "of X and Y, respectively"
    ...
    for p in parts:
        ...
        if not p:
            continue
        # Title-case for matching (Inventories, Other accrued liabilities)
        if p.lower() == "inventories":
            accounts.append("Inventories")
        elif "accrued" in p.lower() and "liab" in p.lower():
...
def format_tatqa_helper_answer(
    ...
    If want_combined_total is None, infer from query: "total" or "combined" -> sum; else return "a, b".
    """
    if not balances_millions:
        return ""
    if want_combined_total is None:
        q = query.strip().lower()
        want_combined_total = "total" in q or "combined" in q or "sum" in q
